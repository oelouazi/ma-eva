...
<!-- Le container principal devient un role="region" accessible -->
<div
	role="region"
	tabindex="0"
...
>
	...
	{#each hearts as heart (heart.id)}
		<span class="absolute text-pink-500 text-2xl animate-float" style="left:{heart.x}px; top:{heart.y}px;">❤️</span>
	{/each}

	<a
		href={`${base}/`}
		class="fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)] right-[calc(env(safe-area-inset-right)+1rem)] inline-flex items-center gap-2 px-5 py-3 rounded-full bg-pink-500 text-white shadow-lg hover:bg-pink-600 active:scale-[0.98] transition"
